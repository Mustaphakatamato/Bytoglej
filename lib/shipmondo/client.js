 ...
 */
export async function getPriceQuote({ carrier, service_type, size_category, from_zip, to_zip, service_point_id }) {
  if (IS_MOCK) {
  ...
  // CreateShipmentRequest-schema: parties[] med postal_code (IKKE sender/receiver),
  // intet carrier_code (carrier udledes af product_code). Required: own_agreement,
  // parcels, product_code, service_codes. Verificeret mod OpenAPI-spec.
  const data = await shipmondoRequest('POST', '/shipments/quote', {
    ...
    parties: [
      { type: 'sender',   postal_code: from_zip || '2100', country_code: 'DK' },
      { type: 'receiver', postal_code: to_zip   || '2100', country_code: 'DK' },
    ],
...
export async function createShipment({ sender, receiver, carrier, service_type, size_category, reference, pickup_point_id }) {
  ...
  const body = {
    ...
    product_code:  serviceCode(carrier, service_type),
    service_codes: 'EMAIL_NT',
    label_format:  'pdf',   // KRÆVES for at labels[] kommer med i svaret
    reference:     reference,
...
/**
 * Henter en allerede-oprettet forsendelses label fra Shipmondo og uploader den til storage.
 * Bruges til backfill af gamle ordrer hvor labelen aldrig blev udtrukket.
 * @returns {Promise<string|null>} public URL eller null
 */
export async function getLabelUrlForShipment(shipmondoShipmentId) {
  if (IS_MOCK || !shipmondoShipmentId) return null;
  const labels = await shipmondoRequest('GET', `/shipments/${shipmondoShipmentId}/labels`);
  const labelBase64 = labels?.[0]?.base64 ?? null;
  return uploadLabelBase64(labelBase64, shipmondoShipmentId);
}
