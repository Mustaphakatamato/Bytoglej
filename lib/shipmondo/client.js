...
/**
 * Uploader en base64-kodet PDF-label til den PRIVATE shipping-labels bucket og
 * returnerer en autoriseret app-route URL (ikke en permanent offentlig URL —
 * fragtlabels indeholder navne+adresser). Route'en /api/shipping/label/<id>
 * autoriserer kalderen og laver en kortlivet signed URL ved kald.
...
async function uploadLabelBase64(base64, shipmentId) {
  ...
  try {
    ...
    return `${base}/api/shipping/label/${encodeURIComponent(shipmentId)}`;
  } catch (e) {
...
/**
 * Henter (og uploader) labelen for en allerede booket forsendelse.
 * Backfill når POST /shipments ikke gav en label med i første omgang.
 * @returns {Promise<string|null>} offentlig label-URL eller null
 */
export async function fetchShipmentLabel(shipmondo_shipment_id) {
  if (IS_MOCK || !shipmondo_shipment_id) return null;
  const labels = await shipmondoRequest('GET', `/shipments/${shipmondo_shipment_id}/labels`);
  const base64 = labels?.[0]?.base64 ?? null;
  return uploadLabelBase64(base64, shipmondo_shipment_id);
}
