...
export async function createShipment({ sender, receiver, carrier, service_type, size_category, reference, pickup_point_id }) {
  ...
  // MIDLERTIDIG DIAGNOSTIK: log svarstrukturen så vi kan se hvor labelen ligger
  console.log('[shipmondo] POST /shipments svar-nøgler:', Object.keys(data),
    '| har label_base64:', !!data.label_base64,
    '| packages[0]-nøgler:', data.packages?.[0] ? Object.keys(data.packages[0]) : null);
...
export async function getLabelUrlForShipment(shipmondoShipmentId) {
  ...
  const full = await shipmondoRequest('GET', `/shipments/${shipmondoShipmentId}`);
  const labelBase64 = full.label_base64 ?? full.packages?.[0]?.label_base64 ?? null;
