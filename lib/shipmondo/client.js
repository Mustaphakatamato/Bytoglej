...
export async function getPriceQuote({ carrier, service_type, size_category, from_zip, to_zip, service_point_id }) {
  ...
  // Returnerer price (inkl. moms) + price_before_vat. Verificeret mod OpenAPI-spec.
  const data = await shipmondoRequest('POST', '/shipments/quote', {
    own_agreement: false,
    sender:   { zipcode: from_zip || '2100', country_code: 'DK' },
    receiver: { zipcode: to_zip || '2100', country_code: 'DK' },
    ...
    ...(service_point_id ? { service_point: { id: service_point_id } } : {}),
  });
