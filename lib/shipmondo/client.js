/**
 * Shipmondo API client
 *
 * Når du har API-credentials fra Shipmondo:
 *   1. Sæt SHIPMONDO_API_KEY i environment variables
 *   2. Sæt SHIPMONDO_MOCK=false (eller fjern variablen)
 *   3. API-dokumentation: https://app.shipmondo.com/api/public/v3/docs
 *
 * I mock-mode returneres realistiske test-data så alt UI og
 * databaselogik kan bygges og testes uden rigtige credentials.
 */

const BASE_URL = 'https://app.shipmondo.com/api/public/v3';
const API_KEY  = process.env.SHIPMONDO_API_KEY;
const IS_MOCK  = !API_KEY || process.env.SHIPMONDO_MOCK === 'true';

// ── Helpers ───────────────────────────────────────────────────

function authHeader() {
  // Shipmondo bruger HTTP Basic Auth med API-nøgle som password
...
export async function createShipment({ sender, receiver, carrier, service_type, size_category, reference, pickup_point_id }) {
  ...
  return {
    ...
    label_pdf_url:    data.labels?.[0]?.link ?? null,
    price_dkk:        data.price?.value ?? null,
  };
}
...
function serviceCode(carrier, service_type) {
  const map = {
    postnord: { service_point: 'PDK_DROPPOINT', home_delivery: 'PDK_HOMEDELIVERY' },
    dao:      { parcel_shop: 'DAO_DROPPOINT',  home_delivery: 'DAO_HOMEDELIVERY' },
    gls:      { parcel_shop: 'GLS_DROPPOINT' },
  };
