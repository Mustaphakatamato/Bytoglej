...
// Fisher-Yates shuffle seeded by a counter — same seed = same order
...
export function useFeedListings(listings, refreshSeed) {
  ...
  const sorted = useMemo(() => {
    ...
    const rest    = shuffle(listings.filter(l => !isNew(l)));
    return [...newOnes, ...rest];
    // refreshSeed is a dependency so the shuffle re-runs on every refresh
