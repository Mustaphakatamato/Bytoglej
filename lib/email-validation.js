...
// Spørger serveren om maildomænet faktisk findes (MX/A-record via DNS). Fanger
// gyldigt-formaterede men ikke-eksisterende domæner som "lifde.dk". Ved fejl
// returneres true (konservativt — vi blokerer ikke ved netværks-/serverproblemer).
export async function checkEmailDeliverable(value) {
  try {
    const r = await fetch('/api/validate-email', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: String(value || '').trim() }),
    });
    if (!r.ok) return true;
