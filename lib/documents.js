...
import { ISSUER, ISSUER_CONFIGURED } from '@/lib/issuer';
...
export async function createDocumentRecord(supa, {
  ...
  // Spærre: udsted ikke bilag før byt&legs rigtige stamdata (CVR) er sat i env.
  if (!ISSUER_CONFIGURED) {
    console.warn('[documents] bilagsgenerering springes over — BYTLEG_CVR er ikke sat (issuer ikke konfigureret).');
    return null;
  }
