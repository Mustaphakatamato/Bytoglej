...
function OpslagInner() {
  ...
  const [group, setGroup] = useState(urlParams.get('group') || '');
...
const [saveSearchModal, setSaveSearchModal] = useState(false);
  ...
  const filteredNormal = useMemo(() => {
    let r = listings.filter(l => {
      ...
      const groupKeys = group ? (NAV_CATEGORY_GROUPS.find(g => g.key === group)?.items.map(i => i.key) || []) : null;
      const matchCategory = !category && !group ? true : category ? l.category === category : groupKeys.includes(l.category);
