...
function stepOf(conv) {
  if (conv.deal_completed) return 3;
  if (!conv.is_handled) return 0;
  if (conv.handled_action === 'order_confirmed') return 1;
  return 1;
}

const STEPS_SHIP   = ['Ny ordre','Bekræftet','Afsendt'];
        ...
        return (
          <div key={lbl} style={{ display:'flex', alignItems:'center', flex: i < labels.length - 1 ? 1 : 'none' }}>
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', minWidth:54 }}>
              <div style={{
                width:22, height:22, borderRadius:'50%', border:`2px solid ${done||active ? PRIMARY : PAPER3}`,
              ...
              <div style={{ fontFamily:FONT, fontSize:9, fontWeight: active?700:500, color: done||active ? PRIMARY : INK3, marginTop:3, whiteSpace:'nowrap' }}>{lbl}</div>
            </div>
            {i < labels.length - 1 && (
...
function TaskCard({ conv, onAction, actionLoading }) {
  ...
  const shipping = conv.delivery_method === 'shipping';
  const router   = useRouter();
  const isNew     = step === 0;
  const isInProg  = step === 1;
  const isDone    = step === 3;
  ...
  return (
    <div style={{ background:'#fff', borderRadius:16, overflow:'hidden', boxShadow:'0 1px 4px rgba(22,34,28,0.08)', border:`1px solid ${PAPER3}`, marginBottom:10 }}>
      ...
      <div style={{ display:'flex', gap:12, padding:'14px 16px 8px', alignItems:'center' }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ fontFamily:FONT, fontSize:12, color:INK3, marginTop:1 }}>
            ...
            {conv.delivery_method && <span> · {shipping ? '📦 Forsendelse' : '📍 Afhentning'}</span>}
          </div>
        </div>
        {isNew && (
        ...
        <div style={{ padding:'8px 16px 14px', display:'flex', gap:8 }}>
          {isNew && (
            <button
              ...
              style={{ flex:1, padding:'10px', borderRadius:99, background: actionLoading===conv.id ? PAPER3 : PRIMARY, color: actionLoading===conv.id ? INK3 : '#fff', border:'none', fontFamily:FONT, fontWeight:700, fontSize:13, cursor: actionLoading===conv.id ? 'default' : 'pointer' }}>
          ...
          {isInProg && shipping && (
            <button
              ...
              style={{ flex:1, padding:'10px', borderRadius:99, background:GREEN_TINT, color:PRIMARY, border:`1.5px solid ${PRIMARY}`, fontFamily:FONT, fontWeight:700, fontSize:13, cursor:'pointer' }}>
              📦 Generer forsendelseslabel →
            </button>
          )}
          <button
            onClick={() => router.push('/beskeder?conv=' + conv.id)}
            style={{ padding:'10px 14px', borderRadius:99, background:PAPER2, color:INK3, border:'none', fontFamily:FONT, fontWeight:600, fontSize:12, cursor:'pointer' }}>
            Chat
          </button>
        </div>
      )}
      {isDone && (
        <div style={{ padding:'8px 16px 14px', display:'flex', gap:8 }}>
          <span style={{ fontFamily:FONT, fontSize:12, color:'#065F46', fontWeight:700, background:'#D1FAE5', borderRadius:99, padding:'6px 14px' }}>✓ Gennemført</span>
          <button
            onClick={() => router.push('/beskeder?conv=' + conv.id)}
            style={{ padding:'8px 14px', borderRadius:99, background:PAPER2, color:INK3, border:'none', fontFamily:FONT, fontWeight:600, fontSize:12, cursor:'pointer', marginLeft:'auto' }}>
            Se chat
          </button>
        </div>
      )}
...
export default function MineOpgaverPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await db.from('conversations')
      .select('id,listing_title,listing_emoji,listing_color,listing_image,initiator_name,is_handled,handled_action,deal_completed,deal_type,delivery_method,last_message_at,owner_name,initiator_institution_id')
      .or(parts.join(','))
  ...
  async function handleAction(conv, action) {
    ...
    try {
      ...
      if (action === 'confirm') {
        ...
        const msg = `✅ ${senderName} har bekræftet din ordre — du modtager snart besked om levering.`;
  ...
  const newOrders = active.filter(t => !t.is_handled);
  const inProg   = active.filter(t => t.is_handled);
          ...
          <div style={{ display:'flex', gap:8, padding:'12px 16px 0' }}>
            {newOrders.length > 0 && (
              <span style={{ background:'#FEF9C3', color:'#92400E', borderRadius:99, fontSize:12, fontWeight:700, padding:'4px 12px' }}>
                {newOrders.length} ny{newOrders.length !== 1 ? 'e' : ''} ordre
              </span>
            )}
            {inProg.length > 0 && (
              <span style={{ background:GREEN_TINT, color:PRIMARY, borderRadius:99, fontSize:12, fontWeight:700, padding:'4px 12px' }}>
                {inProg.length} i gang
              </span>
            )}
          </div>
        )}
          ...
          {loading ? (
            ...
            <div style={{ padding:'60px 20px', textAlign:'center', background:'#fff', borderRadius:16 }}>
              ...
              <div style={{ fontFamily:FONT, fontSize:13, color:INK3 }}>
                {filter==='aktive' ? 'Alle dine handler er gennemførte' : 'Gennemførte handler vises her'}
              </div>
            </div>
          ) : (
            shown.map(conv => (
